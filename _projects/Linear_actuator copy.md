...
- Payload is modeled as a point load near the end of the beam (approximately at \(x=L=150\) cm).
...
*(Note: slope continuity does **not** require the slope to be zero; it only requires the left and right slopes to match.)*
...
- **\(y_{\max} = \dfrac{1.08 \times 10^8}{E I}\) cm**
